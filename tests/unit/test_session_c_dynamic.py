...
def obs(i: int, kind: ObservationKind, load: float) -> EvolutionObservation:
...
def test_trajectory_is_temporal_and_neutral():
    items = (obs(0, ObservationKind.PRESSURE, 1), obs(1, ObservationKind.PRESSURE, 2), obs(2, ObservationKind.AMBIGUITY, 3))
    profile = TrajectoryAnalyzer().profile(items)
...
def test_warning_does_not_imply_evolution_without_persistence():
    ...
    envelope = FieldEnvelopeEstimator().estimate(items, metric_name="load", warning_quantile=0.5, critical_quantile=1.0)
    result = DynamicReadinessEvaluator().evaluate(items, envelope=envelope, current_value=2.0, persistence_floor=0.8)
...
def test_persistent_approach_can_prepare_before_critical():
    items = tuple(obs(i, ObservationKind.PRESSURE, float(i)) for i in range(10))
    envelope = FieldEnvelopeEstimator().estimate(items, metric_name="load")
    result = DynamicReadinessEvaluator().evaluate(items, envelope=envelope, current_value=envelope.warning_limit, persistence_floor=0.5)
    assert result.state == "WARNING"
    assert result.ready is True


def test_topic_is_inferred_from_accumulated_signals():
    items = tuple(obs(i, ObservationKind.PRESSURE, float(i)) for i in range(4)) + (obs(4, ObservationKind.AMBIGUITY, 4),)
