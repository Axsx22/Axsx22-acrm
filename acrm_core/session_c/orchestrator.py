...
from acrm_core.evolution.session_c import EvolutionCandidate, EvolutionDecision, EvolutionObservation, EvolutionSessionC, EvolutionTolerance
...
class SessionCAnalysis:
    """Read-only C-B analysis produced before candidate generation."""
...
class SessionCEngine:
    """Coordinate C-A evidence with governed C-B evolution.

    This engine deliberately has no API for executing or switching runtime
    code. Candidate generation/testing/voting remain isolated callables.
    """
    ...
    def analyze(self) -> SessionCAnalysis:
        ...
        return SessionCAnalysis(readiness=readiness, topic=topic, envelope=envelope)

    def evolve(self) -> EvolutionDecision:
        analysis = self.analyze()
        if not analysis.readiness.ready:
            return EvolutionDecision("WAIT_DYNAMIC_TOLERANCE", None, None, 0, analysis.readiness.reason)
        for observation in self.observations:
            self._governance.record(observation)
        candidate = self._governance.generate_candidate()
        if candidate is None:
            return EvolutionDecision("WAIT_GENERATION", None, None, 0, "candidate generation tolerance has not been reached")
        topic = analysis.topic.dominant.topic if analysis.topic.dominant else "undetermined"
        from acrm_core.evolution.session_c import EvolutionContext
    ...
    def candidate(self) -> EvolutionCandidate | None:
        return self._governance.candidate
