...
class SessionCEngine:
    ...
    def evolve(self) -> EvolutionDecision:
        ...
        # The governance object is persistent across evolve() calls. Record
        # only observations that have not already crossed the canonical C-A
        # boundary; this preserves duplicate-ID rejection without replaying
        # the same evidence on every C-B cycle.
        known_ids = {item.observation_id for item in self._governance.observations}
        ...
        if candidate is None:
            return EvolutionDecision(
                ...
                None,
                0,
                "candidate generation tolerance has not been reached",
            )

        topic = analysis.topic.dominant.topic if analysis.topic.dominant else "undetermined"
        return self._governance.review(
            EvolutionContext(
                topic=topic,
                state={
