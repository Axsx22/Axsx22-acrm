...
Topic is a downstream inference over accumulated observations. It is never
written into the neutral observation layer and never performs promotion.
"""
...
class FieldDrivenTopicEngine:
    """Infer a topic from the distribution of observed field signals."""
    ...
    def infer(self, observations: Iterable[EvolutionObservation]) -> TopicProfile:
        ...
        if not items:
            return TopicProfile(None, ())
        total = len(items)
        signals: list[TopicSignal] = []
        for topic, kinds in self._TOPICS.items():
            count = sum(item.kind in kinds for item in items)
            if count:
                signals.append(TopicSignal(topic, count / total, kinds))
        signals.sort(key=lambda signal: (-signal.support, signal.topic))
